...
function equipmentNameCropRectV312(region, imageWidth, imageHeight) {
  ...
  // v4.0.6 regression target:
  // Keep the NEW DB-icon detection/grid, but restore the OLD icon-anchor
  // equipment-name crop geometry used by the proven OCR path.
  //
  // The only intended change versus the historical OCR path is how the row
  // position is obtained. Once an icon is known, the name crop is derived
  // directly from that icon:
  //   X = icon.x + icon.width + 8
  //   Y = icon.y + icon.height * 0.08
  //   H = icon.height * 0.76
  //   W = icon.width * 7 (clamped to the source image)
  //
  // Do NOT use the v4.0.4/v4.0.5 centered 58% band or the slot-column
  // boundary here. Those were introduced after the historical OCR path and
  // are deliberately excluded from this regression test.
  if (region.sourceType === 'icon-anchor' && region.icon) {
    ...
    if ([ix,iy,iw0,ih0].every(Number.isFinite) && iw0 > 0 && ih0 > 0) {
      ...
      const y = iy + ih0 * 0.08;
      const width = iw0 * 7;
      const height = ih0 * 0.76;
      return {
        ...clampRectToImage({x,y,width,height,source:'v3.1.6-icon-anchor-regression'}, iw, ih),
        pitch,
        source:'v3.1.6-icon-anchor-regression',
        anchorRect:{x:ix,y:iy,width:iw0,height:ih0},
        nameAnchor:{x,y,width,height}
      };
    }
  }
...
const ARMOR_CACHE_KEY='DEV_MWWSKILLSIM_OCR_ARMOR_V1';
const CHARM_CACHE_KEY='DEV_MWWSKILLSIM_OCR_CHARMS_V1';
let armorDB=[], charmDB=[];
...
function normalizeArmorRows(rows){
  return normalizeEquipmentCandidateRows(rows.map(r=>({id:r?.id,gameId:r?.gameId,kind:String(r?.kind||''),name:r?.name})), 'head')
...
async function fetchEquipmentPage(api, offset){
  const u=`${api}?limit=250&offset=${offset}&p=${encodeURIComponent(JSON.stringify({id:true,gameId:true,kind:true,name:true}))}`;
...
async function fetchCharmPage(offset){
  ...
  const u=`${CHARM_API}?limit=250&offset=${offset}&p=${encodeURIComponent(JSON.stringify({id:true,gameId:true,ranks:true}))}`;
...
async function loadArmorDB(force=false){
  if(armorReady&&!force)return armorDB; if(armorPromise&&!force)return armorPromise;
  armorPromise=(async()=>{
    if(!force){try{const c=JSON.parse(localStorage.getItem(ARMOR_CACHE_KEY)||'null');if(Array.isArray(c?.rows)&&c.rows.length){armorDB=c.rows;armorReady=true;return armorDB;}}catch(e){}}
    const all=[];let offset=0;
    for(let page=0;page<100;page++){const part=await fetchEquipmentPage(ARMOR_API,offset);all.push(...part);if(part.length<250)break;offset+=250;}
    armorDB=normalizeArmorRows(all); if(!armorDB.length)throw new Error('防具DBが0件でした'); armorReady=true;
    try{localStorage.setItem(ARMOR_CACHE_KEY,JSON.stringify({savedAt:new Date().toISOString(),rows:armorDB}));}catch(e){} return armorDB;
  })();
...
async function loadCharmDB(force=false){
  if(charmReady&&!force)return charmDB; if(charmPromise&&!force)return charmPromise;
  charmPromise=(async()=>{
    if(!force){try{const c=JSON.parse(localStorage.getItem(CHARM_CACHE_KEY)||'null');if(Array.isArray(c?.rows)&&c.rows.length){charmDB=c.rows;charmReady=true;return charmDB;}}catch(e){}}
    const all=[];let offset=0;
    for(let page=0;page<100;page++){const part=await fetchCharmPage(offset);all.push(...part);if(part.length<250)break;offset+=250;}
    const rows=[]; const seen=new Set();
    for(const c of all){for(const rank of Array.isArray(c?.ranks)?c.ranks:[]){const name=String(rank?.name||'').trim();if(!name)continue;const row={id:c?.id??null,gameId:c?.gameId??null,kind:'charm',name,rankLevel:rank?.level??null};const key=`${row.id}\u0000${row.rankLevel}\u0000${row.name}`;if(seen.has(key))continue;seen.add(key);rows.push(row);}}
    charmDB=rows;if(!charmDB.length)throw new Error('護石DBが0件でした');charmReady=true;
    try{localStorage.setItem(CHARM_CACHE_KEY,JSON.stringify({savedAt:new Date().toISOString(),rows:charmDB}));}catch(e){} return charmDB;
  })();
...
async function fetchWeaponPage(offset){
  const u=`${WEAPON_API}?limit=250&offset=${offset}&p=${encodeURIComponent(JSON.stringify({id:true,gameId:true,kind:true,name:true}))}`;
...
async function loadWeaponDB(force=false){
  ...
  weaponDBPromise=(async()=>{
    if(!force){
      try{
        const cached=JSON.parse(localStorage.getItem(WEAPON_CACHE_KEY)||'null');
        const rows=normalizeWeaponRows(cached?.rows||cached);
        if(rows.length){
          weaponDB=rows;rebuildWeaponGramIndex();weaponDBReady=true;
          setDBStatus(`${rows.length.toLocaleString()}件（端末キャッシュ）`,'ok');
          return weaponDB;
        }
      }catch(e){}
    }
    ...
    for(let page=0;page<100;page++){
      const part=await fetchWeaponPage(offset);
      all.push(...part);setDBStatus(`全件取得中… ${all.length.toLocaleString()}件`,'info');
      if(part.length<250)break;
      offset+=250;
    }
    const rows=normalizeWeaponRows(all);
    if(!rows.length)throw new Error('武器DBが0件でした');
    weaponDB=rows;rebuildWeaponGramIndex();weaponDBReady=true;
    try{localStorage.setItem(WEAPON_CACHE_KEY,JSON.stringify({savedAt:new Date().toISOString(),rows}));}catch(e){}
    setDBStatus(`${rows.length.toLocaleString()}件（最新取得）`,'ok');
    return weaponDB;
  })();
  try{return await weaponDBPromise}catch(e){weaponDBPromise=null;weaponDBReady=false;setDBStatus('取得失敗：'+e.message,'bad');throw e}
  finally{weaponDBPromise=null}
}
...
function extractSubtypeTail(raw){
  ...
  const m=s.match(/(?:^|[^A-Za-z])(80q|bq|Bq|BQ|b|B|6|8|aq|nq|a|A|q|v|V|y|Y|ν|α|β|γ|𝛂|𝛽|𝛾)(?=\s*(?:[^A-Za-z0-9]*)$)/iu);
  return m?m[1]:'';
}
...
function prepareEquipmentOCRCanvas(region,mode='grayscale',band='name'){
  ...
  if(mode==='hsv_white') whiteTextHSV(ctx,out.width,out.height);
  ...
  const pad=(mode==='white_extract'||mode==='hsv_white'||mode==='local_contrast'||mode==='grayscale')?'#000000':'#FFFFFF';
...
function parseSubtypeFromTail(tailStr){
  // Tail OCR often includes UI punctuation/noise after the subtype token.
  // Keep the parser tail-only, but tolerate trailing dashes/digits/noise.
  const clean=String(tailStr||'').normalize('NFKC').replace(/[\r\n]+/g,' ').replace(/\s+/g,' ').trim().toLowerCase();
  if(!clean)return '';
  const token=clean.replace(/[^a-zαβγν𝛂𝛽𝛾0-9]+$/iu,'').trim();
  if(/^(?:80q|bq|β|b|6|𝛽)$/iu.test(token))return 'β';
  if(/^(?:aq|α|a|q|𝛂)$/iu.test(token))return 'α';
...
function aggregateHierarchicalArmorCandidatesV26(rows,items,category,allArmorItems){
  const list=Array.isArray(rows)?rows:[]; const agg=new Map();
  ...
  for(const row of list){
    ...
    const detected=row.subtypeDetected||parseSubtypeFromTail(parseArmorStructure(row.raw,category).tailPart);
...
function aggregateGenericEquipmentCandidates(rows,items){
  ...
  for(const row of rows){
    ...
    ranked.forEach((cand,rank)=>{const rw=rank===0?1:rank===1?.65:.4;const old=agg.get(cand.name)||{...cand,support:0,sumName:0,sumScore:0,bestName:0,confSum:0,total:rows.length};if(rank===0)old.support++;old.sumName+=cand.nameScore*rw;old.sumScore+=cand.score*rw;old.bestName=Math.max(old.bestName,cand.nameScore);old.confSum+=rank===0?row.conf*1:0;old.id=cand.id;old.gameId=cand.gameId;old.kind=cand.kind;agg.set(cand.name,old);});
  }
  return [...agg.values()].map(x=>{const supportRows=Math.max(1,x.support);const avgName=x.sumName/Math.max(1,rows.length);const confAvg=x.support?x.confSum/x.support:0;const supportBonus=x.support===rows.length&&rows.length>=2?.08:x.support>=2?.04:x.support===1&&rows.length===1?.04:0;const total=Math.min(1,.78*avgName+.15*x.bestName+.07*(confAvg/100)+supportBonus);return {...x,avgNameScore:avgName,bestNameScore:x.bestName,confAvg,totalScore:total,supportBonus};}).sort((a,b)=>b.totalScore-a.totalScore);
}
...
async function recognizeEquipmentAll(){
  if(!$('img').src){$('equipmentOCRStatus').textContent='先に画像を選択してください。';return}
  const regions=Array.isArray(window.lastEquipmentRegions)?window.lastEquipmentRegions:[];
  if(regions.length!==9){$('equipmentOCRStatus').textContent='先に「画面構造を自動検出」を実行してください。';return}
  const targets=['mainWeapon','head','chest','arms','waist','legs','charm'];
  const btn=$('recognizeEquipment');btn.disabled=true;
  const started=performance.now();
  try{
    $('equipmentOCRStatus').textContent='装備DBを準備中…';
    await Promise.all([ensureWeaponDB(),loadArmorDB(false),loadCharmDB(false)]);
    ...
    for(const key of targets){
      const region=regions.find(r=>r.key===key); const items=equipmentRowsForSlot(key); const rows=[];
      ...
      for(const [mode,label] of passes){
        ...
        const c=equipmentRegionCanvas(region,mode,'name'); const r=await worker.recognize(c);let raw=cleanOCR(r?.data?.text||'');raw=raw.replace(/(?:メイン武器|サブ武器|頭防具|胴防具|腕防具|腰防具|脚防具|護石|装衣)/g,' ').replace(/\s+/g,' ').trim();const conf=Number(r?.data?.confidence||0);if(raw)rows.push({raw,conf,mode:label});
      ...
      if(key!=='mainWeapon'&&key!=='charm'&&(!ranked.length || ranked[0].avgSeriesScore<.40)){
        for(const [mode,label] of EQUIPMENT_OCR_RESCUE_PASSES){
          ...
          const c=equipmentRegionCanvas(region,mode,'name'); const r=await worker.recognize(c); const raw=cleanOCR(r?.data?.text||''); const conf=Number(r?.data?.confidence||0);
          if(raw)rows.push({mode:label,raw,conf});
    ...
    $('equipmentOCRStatus').textContent=`装備名OCR完了：${output.length}部位 / ${auto}部位が自動確定候補 / ${elapsed.toFixed(1)}秒`;
...
function renderEquipmentResult(x){
  ...
  const raw=x.rows.map(r=>`${r.mode}: ${r.raw} (${Math.round(r.conf)}%)`).join(' / ')||'OCR文字なし';
...
function v400Gray(src){const id=src?.data instanceof ImageData?src.data:src;const a=id?.data,w=Number(id?.width??src?.w),h=Number(id?.height??src?.h);if(!a||!Number.isFinite(w)||!Number.isFinite(h)||w<1||h<1)throw new Error(`グレースケール化する画像サイズが不正です：${w}×${h}`);const g=new Float32Array(w*h);for(let i=0,p=0;i<g.length;i++,p+=4)g[i]=.299*a[p]+.587*a[p+1]+.114*a[p+2];return {g,w,h};}
function v400Downsample(src,factor){const w=Math.max(1,Math.floor(src.w/factor)),h=Math.max(1,Math.floor(src.h/factor));const c=document.createElement('canvas');c.width=w;c.height=h;const ctx=c.getContext('2d',{willReadFrequently:true});const tmp=document.createElement('canvas');tmp.width=src.w;tmp.height=src.h;const tx=tmp.getContext('2d',{willReadFrequently:true});const id=tx.createImageData(src.w,src.h);for(let i=0,p=0;i<src.g.length;i++,p+=4){const v=Math.max(0,Math.min(255,src.g[i]));id.data[p]=id.data[p+1]=id.data[p+2]=v;id.data[p+3]=255;}tx.putImageData(id,0,0);ctx.imageSmoothingEnabled=true;ctx.drawImage(tmp,0,0,w,h);return v400Gray(ctx.getImageData(0,0,w,h));}
function v400Ncc(scene,t,x,y,step=2){if(x<0||y<0||x+t.w>scene.w||y+t.h>scene.h)return -Infinity;let sa=0,sb=0,n=0;for(let j=0;j<t.h;j+=step){const base=(y+j)*scene.w+x;for(let i=0;i<t.w;i+=step){sa+=scene.g[base+i];sb+=t.g[j*t.w+i];n++;}}if(!n)return -Infinity;const ma=sa/n,mb=sb/n;let num=0,da=0,db=0;for(let j=0;j<t.h;j+=step){const base=(y+j)*scene.w+x;for(let i=0;i<t.w;i+=step){const a=scene.g[base+i]-ma,b=t.g[j*t.w+i]-mb;num+=a*b;da+=a*a;db+=b*b;}}const den=Math.sqrt((da+1e-9)*(db+1e-9));return den>0&&Number.isFinite(den)?num/den:-Infinity;}
function v400Template(item,scale){const iw=Number(item.im.naturalWidth||item.im.width),ih=Number(item.im.naturalHeight||item.im.height);const w=Math.max(8,Math.round(iw*scale)),h=Math.max(8,Math.round(ih*scale));const c=document.createElement('canvas');c.width=w;c.height=h;const ctx=c.getContext('2d',{willReadFrequently:true});ctx.imageSmoothingEnabled=true;ctx.drawImage(item.im,0,0,w,h);return v400Gray(ctx.getImageData(0,0,w,h));}
async function v400Match(scene,item,minS=.70,maxS=1.30,stepS=.05){const factor=2,scene2=v400Downsample(scene,factor);let best=null;let guard=0;for(let s=minS;s<=maxS+1e-9&&guard<100;s+=stepS,guard++){const t=v400Template(item,s),t2=v400Downsample(t,factor);if(t2.w>=scene2.w||t2.h>=scene2.h)continue;let coarse=null;const maxX=Math.min(scene2.w-t2.w,Math.round(scene2.w*.38)),maxY=scene2.h-t2.h;for(let y=0;y<=maxY;y+=2)for(let x=0;x<=maxX;x+=2){const score=v400Ncc(scene2,t2,x,y,2);if(Number.isFinite(score)&&(!coarse||score>coarse.score))coarse={score,x,y};}if(coarse){const cx=coarse.x*factor,cy=coarse.y*factor;for(let y=Math.max(0,cy-8);y<=Math.min(scene.h-t.h,cy+8);y++)for(let x=Math.max(0,cx-8);x<=Math.min(scene.w-t.w,cx+8);x++){const score=v400Ncc(scene,t,x,y,2);if(Number.isFinite(score)&&(!best||score>best.score))best={score,x,y,w:t.w,h:t.h,scale:s};}}await new Promise(r=>setTimeout(r,0));}return best;}
async function v400LoadBundledIcons(){
  v400IconItems=[];
  const paths=[
    ['db_icons/main_weapon_sample.png','メイン武器アイコン',0],
    ['db_icons/head_armor_sample.png','頭防具アイコン',2],
    ['db_icons/chest_armor_sample.png','胴防具アイコン',3]
  ];
  for(const [path,name,row] of paths){const r=await fetch(path,{cache:'no-store'});if(!r.ok)throw new Error(`${path} HTTP ${r.status}`);const b=await r.blob();const f=new File([b],path.split('/').pop(),{type:b.type||'image/png'});const im=await v400LoadImage(f);v400IconItems.push({name,im,file:f,row});}
  $('anchorDBStatus').textContent=`DBアイコン：${v400IconItems.length}枚（メイン・頭・胴）を読み込み済み`;
}
